...
class Todo(commands.Cog):
  ...
  async def add(self, ctx, *topic):
    try: 
      ...
      for i in topic:
        msg += i + ' '
      if (ctx.author in user_study_list):
        user_study_list[ctx.author].append(msg)
      else:
        user_study_list[ctx.author] = deque()
        [ctx.author].append(msg)
  ...
  async def add(self, ctx):
    try: 
      if (not user_study_list[ctx.author]):
        msg_embed = discord.Embed(description = 'There are no more topics to cover!', color=0x831b6d)
      else:
        user_study_list[ctx.author].popleft()
        msg_embed = discord.Embed(description = 'Next on the list: ' + user_study_list[ctx.author][0], color=0x831b6d)
        await ctx.author.send(embed = msg_embed)
    except Exception as e:
      print(e)
  ...
  async def clear(self, ctx, limit = -1):
    try: 
      if limit == -1:
        user_study_list[ctx.author].clear()
      else:
  ...
  async def list(self, ctx):
    try: 
      msg = str(user_study_list[ctx.author])
